...
export default function SetupPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await axios.post<{ profile_link: string }>(
                `${API_BASE_URL}/setup`,
                { ...formData, image_url: imageUrl }
            );

            const { profile_link } = response.data;
            setProfileLink(profile_link);

            const slug = profile_link.split('/').pop();
            if (slug) {
                navigate(`/u/${slug}`);
            } else {
            ...
            <div className="bg-black border border-neutral-800 rounded-2xl p-6 max-w-md w-full text-white shadow-xl">
                ...
                {profileLink && (
                    <p className="text-green-500 text-center mb-4">
                        Profile created! View it: <a href={profileLink} className="underline">{profileLink}</a>
                    </p>
                )}
                ...
                <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                    ...
                    {/* Image Upload */}
                    <div>
                        <label htmlFor="image_file" className="block text-sm text-gray-400 mb-1">
                            Profile Image *
                        </label>
                        <input
                            type="file"
